...
def dashboard(request):
    ...
    enabled_slot_keys = settings.enabled_slots or ["breakfast", "lunch", "dinner"]
    ...
    context = {
        ...
        "planned_this_week_count": MealPlanEntry.objects.filter(
            date__range=(week_start, week_end), meal_slot__in=enabled_slot_keys
        ).count(),
        ...
        "today_entries": MealPlanEntry.objects.filter(
            date=today, meal_slot__in=enabled_slot_keys
        ).select_related("recipe"),
        "upcoming_entries": MealPlanEntry.objects.filter(
            date__gt=today, meal_slot__in=enabled_slot_keys
